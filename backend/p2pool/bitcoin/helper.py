...
def getwork(bitcoind, use_getblocktemplate=False, txidcache=None, feecache=None, feefifo=None, known_txs=None):
    ...
    defer.returnValue(dict(
        ...
        bits=bitcoin_data.FloatingIntegerType().unpack(hex_to_bytes(work['bits'])[::-1]) if isinstance(work['bits'], str) else bitcoin_data.FloatingInteger(work['bits']),
        coinbaseflags=hex_to_bytes(work['coinbaseflags']) if 'coinbaseflags' in work else b''.join(hex_to_bytes(x) for x in work['coinbaseaux'].values()) if 'coinbaseaux' in work else b'',
        height=work['height'],
...
def submit_block_rpc(block, ignore_failure, bitcoind, bitcoind_work, net):
    ...
    if bitcoind_work.value['use_getblocktemplate']:
        try:
            result = yield bitcoind.rpc_submitblock(bytes_to_hex((bitcoin_data.block_type if segwit_activated else bitcoin_data.stripped_block_type).pack(block)))
        except jsonrpc.Error_for_code(-32601): # Method not found, for older litecoin versions
            result = yield bitcoind.rpc_getblocktemplate(dict(mode='submit', data=bytes_to_hex(bitcoin_data.block_type.pack(block))))
        success = result is None
    else:
        result = yield bitcoind.rpc_getmemorypool(bytes_to_hex(bitcoin_data.block_type.pack(block)))
        success = result
